...
def graficar_comparativa(series, titulo, archivo):
    ...
    for etiqueta, xs, ys in series:
        ax.plot(xs, ys, marker="o", markersize=7, linewidth=2.3, label=etiqueta)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("tamano de la estructura (n)", fontsize=14)
    ax.set_ylabel("tiempo promedio (microsegundos, escala log)", fontsize=14)
...
def graficar_metodo(datos, implementaciones, metodo, titulo, archivo):
    ...
    for impl in implementaciones:
        ...
        if not xs:
            continue
        ax.plot(xs, ys, marker="o", markersize=7, linewidth=2.3, label=impl)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("tamano de la estructura (n)", fontsize=14)
    ax.set_ylabel("tiempo promedio (microsegundos, escala log)", fontsize=14)
